...
class FalconInterface(object):
    ...
    def handler(self, doc):
        ...
        doc.app.add_route(
            doc.root_uri_absolute(slashes=True), SwaggerDocHandler(), suffix=suffix
        )
        doc.app.add_route(
            doc.root_uri_absolute(slashes=False), SwaggerDocHandler(), suffix=suffix
        )
        ...
        if doc.config_rel_url is None:
            doc.app.add_route(
                doc.swagger_json_uri_absolute, SwaggerConfigHandler(), suffix=suffix
            )
