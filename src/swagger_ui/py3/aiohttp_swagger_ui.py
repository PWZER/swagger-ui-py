...
def api_doc(app, config_path, url_prefix='/api/doc', title='API doc'):
    ...
    async def swagger_index_handler(request):
        return web.Response(text=utils.render_html(url_prefix=url_prefix, title=title), content_type='text/html')

    async def swagger_config_handler(request):
        return web.Response(text=utils.load_swagger_config(config_path=config_path, host=request.host), content_type='application/json')
